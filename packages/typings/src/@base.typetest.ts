...
class DatabaseValueUnit implements KVH.DB.ValueUnit {
  formBytes<T extends KVH.DB.ValueUnit>(this: T, buffer: Uint8Array): T {
...
//     .$defineAliasType<'Address', Address>('Address', new Address())
//     // .$defineAliasType<'Equity', Equity>('Equity', new Equity())
//     .$defineTemplateValue('{senderId:Address}.voteTo.{recipientId:Address}.equity', new Equity())
//     .$defineTemplateValue('{senderId:Address}.transferTo.{zzz:Address}.amount', new Amount())
//     .toBuilder();
